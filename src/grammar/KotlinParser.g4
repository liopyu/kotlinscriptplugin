...
kotlinFile
    : shebangLine? NL* fileAnnotation* packageHeader? importList? (topLevelObject | topLevelExpression)* EOF
    ;
...
typeCheckExpression
    : infixOperation (isOperator type | inOperator expression)
    ;

topLevelJumpExpression
    : THROW NL* expression
    ;
...
classDeclaration
    : modifiers? (CLASS | (FUN NL*)? INTERFACE) NL* simpleIdentifier
      (NL* typeParameters)? (NL* primaryConstructor)? // Optional constructor for simplified syntax in scripts
...
primaryConstructor
    ...
    ; // Made `classParameters` optional for a more concise constructor syntax

classBody
    : LCURL NL* classMemberDeclarations? NL* RCURL
    ; // Allow for an empty `classBody`, common in scripts where class bodies might be sparse
...
classParameter
    ...
    ; // Allow default values for class parameters (common in scripting)
...
classMemberDeclarations
    : (classMemberDeclaration semis? | topLevelExpression semis?)*
    ;
...
variableDeclaration
    : annotation* NL* simpleIdentifier (NL* COLON NL* type)? (NL* ASSIGNMENT NL* expression)?
    ;
...
typeReference
    : userType
    | DYNAMIC
    | AnyType // Support for a more generic 'Any' type if needed
    ;
...
userType
    : simpleUserType (NL* DOT NL* simpleUserType)* // Allows chained user types
    ;
...
block
    : LCURL NL* statements? NL* RCURL NL*
    ;
...
forStatement
    : FOR NL* LPAREN annotation* (variableDeclaration | multiVariableDeclaration)
      IN expression RPAREN NL* controlStructureBody? NL*
    ;
whileStatement
    : WHILE NL* LPAREN expression RPAREN NL* (controlStructureBody | SEMICOLON) NL*
    ;

doWhileStatement
    : DO NL* controlStructureBody? NL* WHILE NL* LPAREN expression RPAREN NL*
    ;
